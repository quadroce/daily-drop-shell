-- Step 1: Create function to migrate user_topic_preferences to preferences
...
BEGIN
  ...
  LOOP
    -- Create preferences record with topic IDs from user_topic_preferences
    INSERT INTO preferences (user_id, selected_topic_ids, selected_language_ids)
    SELECT 
      user_record.user_id,
      ARRAY_AGG(topic_id) as selected_topic_ids,
      COALESCE(
        (SELECT language_prefs FROM profiles WHERE id = user_record.user_id),
        ARRAY[]::text[]
      )::bigint[] as selected_language_ids
    FROM user_topic_preferences 
...
-- Step 3: Add helper function to check if user is following a topic
