-- Fix the feed_get_page_drops RPC to handle pagination correctly
CREATE OR REPLACE FUNCTION public.feed_get_page_drops(
    ...
    p_l1 integer DEFAULT NULL::integer, 
    p_l2 integer DEFAULT NULL::integer
)
RETURNS TABLE(
    id bigint, title text, url text, source_id bigint, image_url text, 
    summary text, published_at timestamp with time zone, language text, 
    tags text[], l1_topic_id integer, l2_topic_id integer, type drop_type, 
    youtube_video_id text, youtube_channel_id text, youtube_thumbnail_url text, 
    source_name text, final_score numeric, reason_for_ranking text
)
LANGUAGE plpgsql
...
BEGIN
    ...
    IF cache_count > 0 THEN
        ...
        -- Get total entries to know when we've reached the end
        SELECT COUNT(*) INTO total_cache_entries
        FROM user_feed_cache ufc
        WHERE ufc.user_id = p_user_id 
        AND ufc.expires_at > now();
        
        -- Return cached results with pagination
        -- Only return up to p_limit items, even if cache has more
        RETURN QUERY
        SELECT 
            d.id,
            d.title,
