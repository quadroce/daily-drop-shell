-- Drop any existing conflicting versions of the function
DROP FUNCTION IF EXISTS public.feed_get_page_drops(uuid,int,text,text,bigint,bigint);
DROP FUNCTION IF EXISTS public.feed_get_page_drops(uuid,int,text,text,integer,integer);

-- Recreate the function with consistent types (using bigint for topic IDs)
...
begin
  ...
  with base as (
    select
      ...
      d.published_at, d.language, d.tags, 
      d.l1_topic_id::bigint, d.l2_topic_id::bigint,
