-- Ottimizzazioni performance feed
...
-- Rimuovi tutti i possibili job esistenti (ignorando errori se non esistono)
SELECT cron.unschedule('background-feed-ranking-daily') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'background-feed-ranking-daily');
SELECT cron.unschedule('daily-feed-ranking') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'daily-feed-ranking');
...
-- Ottimizza indici per query fallback
...
WHERE tag_done = true AND published_at >= (now() - interval '7 days');
